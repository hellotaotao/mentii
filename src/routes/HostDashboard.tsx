    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-slate-100 px-6 text-slate-900">
        <section className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-lg">
          ...
          <p className="mt-4 text-sm text-slate-600">Fetching the rooms you created so you can resume instantly.</p>
        </section>
      </main>
    )
  }
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-slate-100 px-6 text-slate-900">
        <section className="w-full max-w-md rounded-3xl border border-rose-300/50 bg-rose-50 p-8 shadow-lg">
          ...
          <p className="mt-4 text-sm text-rose-700">{errorMessage ?? 'Try refreshing this page.'}</p>
        </section>
      </main>
    )
  }
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-6 py-10 text-slate-900">
      <section className="mx-auto w-full max-w-5xl space-y-6">
        ...
        {isCreateDialogOpen ? (
          <section className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 px-4" role="dialog" aria-modal="true">
            <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
              <p className="text-sm uppercase tracking-[0.3em] text-cyan-600">New room</p>
              <h2 className="mt-2 text-2xl font-semibold text-slate-900">Name your room</h2>
              <p className="mt-2 text-sm text-slate-600">This name can be changed later inside the room editor.</p>
        ...
        {rooms.length === 0 ? (
          <section className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center">
            <h2 className="text-2xl font-semibold text-slate-900">No rooms yet</h2>
            <p className="mt-3 text-sm text-slate-600">Create your first room to start building slides and collecting votes.</p>
          </section>
        ) : (
              ...
              {rooms.map((room) => {
                ...
                return (
                  <li className="flex flex-col gap-4 px-5 py-4 md:flex-row md:items-center md:justify-between" key={room.id}>
                    <div>
                      <p className="text-xs uppercase tracking-[0.25em] text-slate-500">Room</p>
                      <p className="mt-2 text-xl font-semibold text-slate-900">{room.name}</p>
                      <p className="mt-1 text-xs text-slate-500">{`Room ID ${room.id}`}</p>
                      <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-slate-600">
                        <span>{`Code ${formatSessionCode(room.code)}`}</span>
                        <span>{`Created ${formatCreatedAt(room.created_at, dateFormatter)}`}</span>
                        <span className="rounded-full border border-slate-300 bg-slate-100 px-3 py-1 text-xs uppercase tracking-[0.2em] text-slate-700">
                          {room.state}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <button
                        className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-rose-300 hover:text-rose-700 disabled:cursor-not-allowed disabled:opacity-50"
                        disabled={isDeleting}
                        onClick={() => {
